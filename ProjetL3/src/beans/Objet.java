...
public class Objet {
	...
	public void rendreOjet(int id) throws Exception
	{
		...
		try {
			...
			ResultSet resultat;
			try{
				String sqlToObjet = "update objet set qtiteRest = qtiteRest + (select qtite_emprunt from emprunt where id = "+id+") where id = (select id_objet from emprunt where id = "+id+");";
	...
	public void listeEmprunt(HttpServletRequest request) {
		...
		try {
			...
			ResultSet resultat;
			///////////////////  A rendre  ////////////////////////////
			resultat = statement.executeQuery( "select emprunt.id as id,objet.intitule as nom, qtite_emprunt, rendu from "
					+ "emprunt join objet on id_objet = objet.id where nom_User = '"+username+"' and rendu = false" );
